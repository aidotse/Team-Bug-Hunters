...
import os
import time
from merge_and_tile import get_tiles_array, create_tiles_in_tensorflow
...
PATH = os.path.join(os.path.dirname("images_for_preview","40x images"))
...
train_dataset = tf.data.Dataset.list_files(PATH+'train/*.tif')
train_dataset = train_dataset.map(load_image_train,
                                  num_parallel_calls=tf.data.experimental.AUTOTUNE)
train_dataset = train_dataset.shuffle(BUFFER_SIZE)
train_dataset = train_dataset.batch(BATCH_SIZE)

test_dataset = tf.data.Dataset.list_files(PATH+'test/*.tif')
...
def train_step(input_image, target, epoch):
  with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
    ...
    tiles = tf.py_function(
      create_tiles_in_tensorflow, [input_image], [tf.float32], name="create_tiles"
    )


    gen_tiles = []
    for i in range(len(tiles)):
      print("shape input tile: ",tf.expand_dims(tiles[i],axis=0).shape)
      gen_output = generator(tf.expand_dims(tiles[i], axis=0),training=True)
      gen_tiles.append(gen_output)
    ...
    # End of tiling loop and merge method call
...
def predict_images(number_predictions):
  for inp, tar in test_dataset.take(number_predictions):
